...
def build_parser():
    p = argparse.ArgumentParser(
        description="Run offline CMNIST correction using one fixed observational population."
    )
    p.add_argument("--output", type=str, default="examples/cmnist/results/offline")
    ...
    # Conditional outcome nuisance: exact update budget, fixed observed empirical base.
...
@torch.no_grad()
def sample_empirical_chunked(base_by_arm, arm: int, n: int, chunk: int):
    """Sample with replacement from one fixed arm-specific observed-Y pool."""
    base = base_by_arm[int(arm)]
    parts = []
    for start in range(0, int(n), int(chunk)):
        count = min(int(chunk), int(n) - start)
        indices = torch.randint(len(base), (count,), device=base.device)
        parts.append(base[indices].detach().cpu())
    return torch.cat(parts, dim=0)
...
def main():
    ...
    # ------------------------------------------------------------------
    # 2. One fixed offline empirical base shared by nuisance and targets.
    # ------------------------------------------------------------------
    treatment = A.reshape(-1).long()
    empirical_bases = {arm: Y[treatment == arm] for arm in (0, 1)}
    if any(len(base) == 0 for base in empirical_bases.values()):
        raise RuntimeError("The fixed observational population is missing a treatment arm.")
    print(
        "Offline empirical base sizes:",
        {f"arm{arm}": len(empirical_bases[arm]) for arm in (0, 1)},
    )
    ...
    #    Its FM base is sampled from Y[A=a] in this same fixed population.
    # ------------------------------------------------------------------
    offline_nuisance_cfg = ConditionalFlowFMConfig(
        dim_y=1,
        dim_x=1,
        lr=args.nuisance_lr,
    ...
    offline_nuisance = ConditionalFlowFM(offline_nuisance_cfg, device=device)
    offline_nuisance.fit_iterations(
        X,
        A,
        Y,
    ...
    # Both target variants sample their bases from the identical fixed Y[A=a] pools.
    ...
    # ------------------------------------------------------------------
    # 5. Fresh interventional references; learned/source samples use fixed bases.
    # ------------------------------------------------------------------
    truth = {}
    source = {}
    ...
    for arm in (0, 1):
        ...
        source[arm] = sample_empirical_chunked(
            empirical_bases, arm, args.eval_n, args.sample_chunk
        )
    ...
    for arm in (0, 1):
        ...
        checkpoint_bases[arm] = sample_empirical_chunked(
            empirical_bases,
            arm,
    ...
    trajectory_bases = {
        arm: sample_empirical_chunked(
            empirical_bases, arm, args.trajectory_n, args.trajectory_chunk
        )
        for arm in (0, 1)
    }
    ...
    config_payload.update(
        {
            ...
            "study_mode": "offline_empirical",
            "nuisance_base": "fixed_observational_Y_stratified_by_A",
            "target_base": "fixed_observational_Y_stratified_by_A",
            "fresh_generator_base_used": False,
            "empirical_base_n_by_arm": {
                f"arm{arm}": len(empirical_bases[arm]) for arm in (0, 1)
            },
    ...
    save_json(
        {
            ...
            "base_reconstruction": "Reconstruct the population, then use Y[A == arm].",
